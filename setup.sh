...
PROJECT_USER="${ACP_PROJECT_USER:-${USER:-$(id -un)}}"
...
runs the guarded fresh-appliance installer, launches the local Plexamp claim flow
when a new player needs claiming, then captures that claimed player name as the
appliance commissioning baseline and selects the managed Plexamp audio output.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --audio) [[ $# -ge 2 ]] || fail '--audio requires direct or eq'; AUDIO_PROFILE="$2"; shift 2 ;;
        --weather-observations) [[ $# -ge 2 ]] || fail '--weather-observations requires ecowitt-push or weather-underground'; WEATHER_OBSERVATIONS="$2"; WEATHER_OBSERVATIONS_EXPLICIT=true; shift 2 ;;
        --project-user) [[ $# -ge 2 ]] || fail '--project-user requires a user'; PROJECT_USER="$2"; shift 2 ;;
        --camilladsp-binary) [[ $# -ge 2 ]] || fail '--camilladsp-binary requires a path'; CAMILLA_BINARY="$2"; shift 2 ;;
        --wu-station-id) [[ $# -ge 2 ]] || fail '--wu-station-id requires a station ID'; WU_STATION_ID="$2"; shift 2 ;;
        --wu-api-key-file) [[ $# -ge 2 ]] || fail '--wu-api-key-file requires a path'; WU_API_KEY_FILE="$2"; shift 2 ;;
        --dashboard-url) [[ $# -ge 2 ]] || fail '--dashboard-url requires a URL'; DASHBOARD_URL="${2%/}"; shift 2 ;;
...
case "$AUDIO_PROFILE" in direct|eq) ;; *) fail "unsupported audio profile '$AUDIO_PROFILE'" ;; esac
case "$WEATHER_OBSERVATIONS" in ecowitt-push|weather-underground) ;; *) fail "unsupported weather observations provider '$WEATHER_OBSERVATIONS'" ;; esac
...
if [[ "$PRESERVE_WEATHER_OBSERVATIONS" == true ]]; then
    [[ -z "$WU_STATION_ID" && -z "$WU_API_KEY_FILE" ]] || fail 'Weather Underground station/key options on a repeat setup require an explicit --weather-observations choice'
    ...
    [[ -n "$WU_API_KEY_FILE" && -f "$WU_API_KEY_FILE" && ! -L "$WU_API_KEY_FILE" && -r "$WU_API_KEY_FILE" ]] || fail 'Weather Underground requires a readable --wu-api-key-file'
else
    [[ -z "$WU_STATION_ID" && -z "$WU_API_KEY_FILE" ]] || fail 'Weather Underground station/key options require --weather-observations weather-underground'
fi
...
if [[ "$AUDIO_PROFILE" == eq && -z "$CAMILLA_BINARY" ]]; then
    ...
    bash "$REPO_ROOT/scripts/fetch-camilladsp-4.1.3.sh" --activate --confirm FETCH-CAMILLADSP-4.1.3
...
plexamp_claim_state_present() {
    [[ -d "$PLEXAMP_SETTINGS" ]] && find "$PLEXAMP_SETTINGS" -maxdepth 1 -type f -print -quit 2>/dev/null | grep -q .
}
...
EOF
    set +e
    trap ':' INT
    (cd "$PLEXAMP_HOME" && "$NODE_BIN" js/index.js)
    claim_rc=$?
    trap - INT
    set -e
...
run_guarded_installer() {
    local args=(bash "$REPO_ROOT/appliance-installer.sh" --fresh-bootstrap --audio "$AUDIO_PROFILE" --project-user "$PROJECT_USER" --dashboard-url "$DASHBOARD_URL" --apply --confirm APPLY-A-CLOCKWORK-PLEX)
...
run_plexamp_commissioning() {
    echo
    echo '[A Clockwork Plex] Capturing/verifying Plexamp player-name baseline and managed audio output...'
...
while true; do
    ...
    case "$installer_rc" in
        0)
            if ! run_plexamp_commissioning; then
                cat >&2 <<'EOF'

[A Clockwork Plex] The guarded appliance installation committed, but Plexamp commissioning did not verify.
No Plex authentication/session data was changed by that commissioning owner. Re-run `bash setup.sh`
once Plexamp is running and the managed `A Clockwork Plex - Plexamp` output is available.
EOF
                exit 2
            fi
            cat <<'EOF'
...
Plexamp's claimed player name is recorded as this appliance's reset baseline and the managed audio output is verified.
