...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/test-stage-c23-current-package-managed-file-rollback.sh

  bash scripts/test-stage-c23-current-package-managed-file-rollback.sh \
    --rehearse-managed-file-rollback \
    --confirm STAGE-C23-CURRENT-PACKAGE-MANAGED-FILES-EXACT-ROLLBACK \
    ...
    --evidence-root /var/tmp/a-clockwork-plex-stage-c23-current-package-managed-file-rollback.XXXXXX
...
proves DAC release, atomically writes all 28 accepted managed files, verifies
the installed identities, then removes the exact installed inodes and only
transaction-created directories before restoring the accepted appliance state.

Systemd reload, route selection, mixer writes, approval publication, CamillaDSP
startup, audio probes, commit and activation remain blocked throughout.
EOF
}
...
if [[ "$MODE" == "prepare" ]]; then
  cat <<'EOF'
A Clockwork Plex Stage C23 current-package managed-file rollback rehearsal is prepared.

Prepare-only invoked no sudo, performed no host observation, stopped no service,
wrote no production file and created no evidence root, lock or transaction.
...
The intended separately approved inputs are:
  ...
  EVIDENCE="$(mktemp -d /var/tmp/a-clockwork-plex-stage-c23-current-package-managed-file-rollback.XXXXXX)"
  chmod 0700 "$EVIDENCE"

Do not run guarded mode without a new explicit approval. Plexamp, AirPlay and
the dashboard will be temporarily unavailable, and 28 fixed production files
will exist briefly before mandatory exact rollback. SSH remains outside the
application boundary. Any rollback or restoration failure deliberately retains
the canonical lock and transaction for inspection; do not clean them manually.
EOF
  exit 0
fi
...
[[ "$CONFIRMATION" == "$REQUIRED_CONFIRMATION" ]] || {
  echo "Guarded Stage C23 rehearsal requires --confirm $REQUIRED_CONFIRMATION" >&2
  exit 64
}
...
exec sudo env \
  ...
  python3 -B -m stage_c_transaction.current_package_managed_file_rollback_rehearsal_v9 \
