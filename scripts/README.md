...
| `scripts/inspect-plexamp-browser-storage.py` | Inventories browser-local persistence metadata for the disposable Plexamp profile: bounded Local/Session Storage key-family counts plus IndexedDB database/object-store names. It never reads Web Storage values, IndexedDB records or opens IndexedDB transactions. | With the disposable loopback-debug Chromium profile open, run `python3 scripts/inspect-plexamp-browser-storage.py` — **read-only developer diagnostic**; never target the production kiosk profile. |
