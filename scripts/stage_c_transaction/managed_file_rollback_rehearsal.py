...
"""Stage C18 root-owned managed-file install and exact-rollback rehearsal."""
...
from .service_quiescence_rehearsal import (
    EXPECTED_CHECKS as STAGE_C17_CHECKS,
)
...
EVIDENCE_PREFIX = "a-clockwork-plex-stage-c18-managed-file-rollback."
EXPECTED_CHECKS = (
    "root-scope",
    "input-replay",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Stage C18: repeat the accepted C17 prefix, atomically install "
            "the twelve managed files, keep systemd reload and route selection "
            "blocked, restore the authoritative filesystem snapshot, restore "
            "the application services and close the exact-rollback rehearsal."
        )
    )
    parser.add_argument("--package-root", required=True, type=Path)
    parser.add_argument("--stage-c17-root", required=True, type=Path)
...
def validate_evidence_root(raw: Path, invoking_uid: int) -> Path:
    root = _validate_owned_root(
        ...
        label="Stage C18 evidence root",
    )
    if any(root.iterdir()):
        raise SystemExit("Stage C18 evidence root must be empty")
    return root


def validate_stage_c17(root: Path, invoking_uid: int) -> Path:
    root = _validate_owned_root(
        root,
        prefix="a-clockwork-plex-stage-c17-service-quiescence.",
        invoking_uid=invoking_uid,
        label="Stage C17 evidence",
    )
    _validate_evidence_manifest(root, "Stage C17")
    rows = _read_tsv(root / "results.tsv")
    if tuple(row.get("check", "") for row in rows) != STAGE_C17_CHECKS:
        raise SystemExit(
            "Stage C17 evidence does not contain the exact thirty-five checks"
        )
    if any(row.get("result") != "PASS" for row in rows):
        raise SystemExit("Stage C17 evidence contains a non-PASS result")
    blocked = _read_tsv(root / "blocked-operations.tsv")
    if len(blocked) != 14 or any(
        row.get("state") != "blocked" for row in blocked
    ):
        raise SystemExit("Stage C17 blocked-operation evidence changed")
    identity = {
    ...
    for key, expected in required_identity.items():
        if identity.get(key) != expected:
            raise SystemExit(f"Stage C17 identity contract changed: {key}")
    for name in (
        ...
        if path.is_symlink() or not path.is_dir():
            raise SystemExit(f"Stage C17 review evidence is missing: {name}")
    readiness = root / "restoration-readiness.tsv"
    if readiness.is_symlink() or not readiness.is_file():
        raise SystemExit("successful corrected Stage C17 readiness evidence is missing")
    ...
    for marker in (
        ...
        if marker not in report:
            raise SystemExit(f"Stage C17 report contract is missing: {marker}")
    return root
...
def prove_blocked_operations(
    adapter: ManagedFileRollbackRehearsalAdapterV4,
    *,
    ...
    if observed != expected or len(rows) != BLOCKED_V4_COUNT:
        raise SystemExit(
            "blocked-operation coverage changed: "
            f"expected={BLOCKED_V4_COUNT} observed={len(rows)}"
        )
    return rows
...
def write_identity(
    ...
    *,
    ...
    output.write_text(
        ...
        "mutation_started\ttrue\n"
        f"managed_files_installed\t{str(managed_files_installed).lower()}\n"
        f"filesystem_restored\t{str(filesystem_restored).lower()}\n"
        f"services_restored\t{str(services_restored).lower()}\n"
...
def write_report(
    ...
    output.write_text(
        f"""A Clockwork Plex Stage C18 managed-file installation and exact-rollback rehearsal
...
V4 permitted operations: 25
V4 blocked operations: {blocked_count}
...
Proved:
- successful physical Stage C17 evidence and exact Stage C1 package replay
...
- active direct ALSA route remained unchanged during installation
- systemd reload, route selection, managed services, probes and commit remained blocked
- exact managed-file and created-directory rollback before service restoration
...
- v2 pre-mutation abort and v3 service-only closure refused after file mutation
- typed v4 exact-rollback closure removed the transaction before lock release
...
- installation commit
- post-reload automatic rollback
...
def main() -> int:
    ...
    if args.confirm != REQUIRED_CONFIRMATION:
        raise SystemExit(
            f"Stage C18 rehearsal requires --confirm {REQUIRED_CONFIRMATION}"
        )
    ...
    stage_c17_root = validate_stage_c17(args.stage_c17_root, invoking_uid)
    evidence_root = validate_evidence_root(args.evidence_root, invoking_uid)
    inputs = {
        "stage-c1": tree_fingerprint(package_root),
        "stage-c17": tree_fingerprint(stage_c17_root),
    }
    ...
    try:
        ...
        append_result(
            results,
            "input-replay",
            "Stage C1 package and successful physical Stage C17 evidence replayed",
        )

        with ManagedFileRollbackRehearsalAdapterV4(
            package_root,
            invoking_user,
            evidence_root,
        ) as adapter:
            if not isinstance(adapter, ProductionAdapterV4):
                raise SystemExit(
                    "Stage C18 adapter does not conform to ProductionAdapterV4"
                )
            append_result(
                results,
                "protocol-conformance",
                (
                    "adapter exposes twenty-two v1 operations plus v2, v3 and "
                    "v4 lifecycle methods"
                ),
            )
            ...
            if active_apps != {
                ...
                raise SystemExit("Stage C18 requires all three application services active")
            append_result(
                results,
                "service-snapshot",
            ...
            append_result(
                results,
                "blocked-operation-boundary",
                f"all {len(blocked)} reload, route, audio, commit and later restore operations refused exactly",
            )
            ...
            append_event(events, 20, "application-services-stopped", "dashboard,shairport,plexamp")
            ...
            _expect_blocked(
                critical_rows,
                AdapterOperation.RELOAD_SYSTEMD,
                lambda: adapter.reload_systemd(transaction.transaction),
            )
            _expect_blocked(
                critical_rows,
                AdapterOperation.SELECT_SPLIT_BUS_ROUTE,
            ...
            append_result(
                results,
                "post-install-boundary",
                "systemd reload and active route selection remained blocked after file installation",
            )
            ...
            append_result(
                results,
                "exact-filesystem-rollback",
                "all installed files and transaction-created directories restored to the authoritative snapshot",
            )
            ...
            append_result(
                results,
                "application-service-restoration",
                "captured application service state restored exactly after filesystem rollback",
            )
            ...
            append_result(
                results,
                "exact-restoration-boundary",
                "managed-file rehearsal ended with the accepted direct appliance state restored",
            )
            ...
            append_result(
                results,
                "pre-mutation-abort-refusal",
                "v2 pre-mutation abort refused after the service and file mutation boundaries",
            )
            ...
            if (
                ...
                raise SystemExit("v3 service-only closure did not refuse after file mutation")
            append_result(
                results,
                "service-only-closure-refusal",
                "v3 service-only closure refused the managed-file mutation history",
            )
            ...
            close_result = adapter.close_exact_rollback_rehearsal_transaction(
                transaction.transaction
            )
            if (
                ...
                raise SystemExit(
                    f"v4 exact-rollback closure failed: {close_result.detail}"
                )
            ...
            append_result(
                results,
                "candidate-evidence-copy",
                f"validated candidate and managed-file rehearsal retained non-authoritatively at {candidate_copy}",
            )
            append_result(
                results,
                "exact-rollback-close-v4",
                "typed v4 closure accepted only the adapter-generated exact-rollback transaction",
            )
            if (
                ...
                raise SystemExit("v4 closure did not finish exact transaction cleanup")
            append_result(
                results,
                "exact-transaction-cleanup",
            ...
            append_result(
                results,
                "production-lock-released",
                "exact production lock removed only after v4 exact-rollback closure",
            )
        ...
        for label, root in (
            ...
            ("stage-c17", stage_c17_root),
        ):
            if tree_fingerprint(root) != inputs[label]:
                raise SystemExit(f"{label} input changed during Stage C18")
        append_result(
            results,
            "input-integrity",
            "Stage C1 and Stage C17 input trees remained unchanged",
        )
        ...
        append_result(
            results,
            "activation-interface",
            "absent; exact rollback completed before any reload, route selection, managed service, audio probe or commit",
        )
        write_evidence_manifest(evidence_root)
        observed = tuple(
            line.split("\t", 1)[0]
        ...
        if observed != EXPECTED_CHECKS:
            raise SystemExit(f"unexpected Stage C18 result order: {observed}")
        completed = True
    finally:
    ...
    if not completed:
        raise SystemExit("Stage C18 managed-file exact-rollback rehearsal did not complete")
    print(
        f"""
A Clockwork Plex Stage C18 managed-file exact-rollback rehearsal passed.
...
The twelve managed files were installed atomically while the services and DAC
were quiesced, then removed through the authoritative snapshot before systemd
reload or route selection. The exact application and audio state was restored,
the transaction closed and the production lock released. Persistent Stage C
