...
"""Stage C23 current-package managed-file install and exact rollback rehearsal."""
...
from .current_package_managed_file_rollback_adapter_v9 import (
    ...
    apply_current_managed_file_identity_contract_v9,
)
...
from .stage_c22_evidence_identity import (
    ACCEPTED_STAGE_C22_ROOT,
    validate_blocked_boundaries,
    validate_identity as validate_stage_c22_identity,
    validate_input_binding as validate_stage_c22_input_binding,
    validate_report as validate_stage_c22_report,
    validate_results as validate_stage_c22_results,
)


REQUIRED_CONFIRMATION = "STAGE-C23-CURRENT-PACKAGE-MANAGED-FILES-EXACT-ROLLBACK"
EVIDENCE_PREFIX = "a-clockwork-plex-stage-c23-current-package-managed-file-rollback."
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Install the accepted 28-file package only while the application "
            "services and DAC are quiesced, keep reload/routing/activation "
            "blocked, then restore the exact filesystem and appliance state."
        )
    )
...
def validate_evidence_root(raw: Path, invoking_uid: int) -> Path:
    root = _validate_owned_root(
        ...
        label="Stage C23 evidence root",
    )
    if any(root.iterdir()):
        raise SystemExit("Stage C23 evidence root must be empty")
    return root
...
def validate_stage_c22_evidence(raw: Path, invoking_uid: int) -> Path:
    root = _validate_owned_root(
        raw,
        prefix="a-clockwork-plex-stage-c22-current-package-service-quiescence.",
        invoking_uid=invoking_uid,
        label="accepted Stage C22 evidence",
    )
    if root != ACCEPTED_STAGE_C22_ROOT:
        raise SystemExit("Stage C23 accepts only the exact retained Stage C22 root")
    _assert_regular_tree(root, "accepted Stage C22 evidence")
    _validate_evidence_manifest(root, "Stage C22")
    manifest = root / "evidence-manifest.tsv"
    if sha256(manifest) != STAGE_C22_EVIDENCE_MANIFEST_SHA256:
        raise SystemExit("Stage C22 accepted evidence-manifest digest changed")
    rows = sum(1 for _ in manifest.open("r", encoding="utf-8"))
    entries = len(_read_tsv(manifest))
    if rows != STAGE_C22_MANIFEST_ROWS or entries != STAGE_C22_MANIFEST_ENTRIES:
        raise SystemExit("Stage C22 accepted manifest shape changed")
    validate_stage_c22_results(_read_tsv(root / "results.tsv"))
    validate_stage_c22_input_binding(
        _rows_by_key(root / "input-binding.tsv", "item", "value")
    )
    validate_stage_c22_identity(
        _rows_by_key(root / "identity.tsv", "item", "value")
    )
    validate_blocked_boundaries(root)
    validate_stage_c22_report((root / "report.txt").read_text(encoding="utf-8"))
    return root
...
def write_input_binding(
    ...
    output.write_text(
        ...
        f"stage_c22_manifest_sha256\t{STAGE_C22_EVIDENCE_MANIFEST_SHA256}\n"
        f"stage_c22_manifest_rows\t{STAGE_C22_MANIFEST_ROWS}\n"
        f"stage_c22_manifest_entries\t{STAGE_C22_MANIFEST_ENTRIES}\n"
...
def write_report(
    ...
    output.write_text(
        f"""A Clockwork Plex Stage C23 current-package managed-file exact-rollback rehearsal
...
Accepted Stage C22 manifest: {STAGE_C22_EVIDENCE_MANIFEST_SHA256}
...
Proved:
- exact package, baseline, Stage C21 and immutable Stage C22 evidence replay
...
- installed paths, modes, owners and digests matched the transaction candidate
- systemd reload, route selection, managed services, probes, commit and approvals stayed blocked
- exact installed inodes and only transaction-created directories removed
...
- systemd daemon reload with installed units
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.confirm != REQUIRED_CONFIRMATION:
        raise SystemExit(f"Stage C23 requires --confirm {REQUIRED_CONFIRMATION}")
    ...
    try:
        ...
        append_result(results, "root-scope", f"root writes constrained to {evidence_root}, the canonical lock, one disposable transaction and the 28 fixed manifest destinations")
        ...
        append_result(results, "stage-c22-evidence-replay", "accepted 41-check Stage C22 evidence replayed at immutable manifest identity")
        append_result(results, "pre-lock-live-baseline", "fresh fixed read-only observation matches the accepted appliance state")
        write_input_binding(evidence_root / "input-binding.tsv", package_root, baseline_root, stage_c21_root, stage_c22_root)

        with CurrentPackageManagedFileRollbackAdapterV9(
            package_root, invoking_user, evidence_root
        ) as adapter:
            if not isinstance(adapter, ProductionAdapterV3):
                raise SystemExit("Stage C23 adapter does not conform to ProductionAdapterV3")
            if isinstance(adapter, ProductionAdapterV7):
                raise SystemExit("Stage C23 adapter unexpectedly exposes approval-capable v7")
            append_result(results, "protocol-conformance", "current-package validation, reversible service mutation and one typed 28-file exact-rollback closure; no approval methods")
            ...
            if not transaction.transaction.value.startswith(CURRENT_MANAGED_TRANSACTION_PREFIX_V9) or not transaction.snapshot.value.startswith(CURRENT_MANAGED_SNAPSHOT_PREFIX_V9):
                raise SystemExit("Stage C23 transaction identity prefix changed")
            ...
            blocked = prove_blocked_operations(adapter, transaction=transaction.transaction, services=services, mixer=mixer)
            write_blocked_operations(evidence_root / "blocked-operations.tsv", blocked)
            append_result(results, "blocked-operation-boundary", f"all {len(blocked)} non-C23 ordinary operations refused exactly")
            ...
            append_result(results, "pre-mutation-boundary", "no service, DAC, file, route, mixer, approval or audio mutation began")
            ...
            if install_evidence.get("installed_file_count") != "28" or install_evidence.get("payload_file_count") != "27":
                raise SystemExit("Stage C23 installed-file receipt changed")
            ...
            _expect_blocked(critical_rows, AdapterOperation.RELOAD_SYSTEMD, lambda: adapter.reload_systemd(transaction.transaction))
            _expect_blocked(critical_rows, AdapterOperation.SELECT_SPLIT_BUS_ROUTE, lambda: adapter.select_split_bus_route(transaction.transaction))
            if len(critical_rows) != 2:
                raise SystemExit("post-install reload/route boundary was not exact")
            append_result(results, "post-install-boundary", "systemd reload and active route selection remained blocked after file installation")

            rollback_result = adapter.restore_exact_snapshot(transaction.transaction, transaction.snapshot)
            require_receipt(rollback_result, AdapterOperation.RESTORE_EXACT_SNAPSHOT)
            append_result(results, "exact-filesystem-rollback", "all installed inodes and transaction-created directories restored to the authoritative snapshot")
            ...
            append_result(results, "application-service-restoration", "captured application service state restored exactly after filesystem rollback")
            ...
            verify_result = adapter.verify_exact_rollback(transaction.transaction, transaction.snapshot)
            require_receipt(verify_result, AdapterOperation.VERIFY_EXACT_ROLLBACK)
            append_result(results, "exact-rollback-verification", "zero filesystem, service, route, mixer, loopback or DAC mismatch remained")
            append_result(results, "exact-restoration-boundary", "28-file rehearsal ended with the accepted direct appliance state restored")
            ...
            append_result(results, "pre-mutation-abort-refusal", "v2 pre-mutation abort refused after service and file mutation")

            service_close_result = adapter.close_restored_rehearsal_transaction(transaction.transaction)
            if service_close_result.operation is not RestoredRehearsalLifecycleOperation.CLOSE_RESTORED_REHEARSAL_TRANSACTION or service_close_result.status is not AdapterStatus.FAIL:
                raise SystemExit("service-only closure did not refuse after file mutation")
            append_result(results, "service-only-closure-refusal", "service-only closure refused the 28-file mutation history")
            ...
            close_result = adapter.close_current_package_exact_rollback_rehearsal(transaction.transaction)
            if close_result.operation is not CurrentPackageExactRollbackLifecycleOperationV9.CLOSE_CURRENT_PACKAGE_EXACT_ROLLBACK_REHEARSAL or close_result.status is not AdapterStatus.PASS or close_result.payload is None:
                raise SystemExit(f"Stage C23 exact-rollback closure failed: {close_result.detail}")
            ...
            if not candidate_copy.is_dir() or not transaction_copy.is_dir():
                raise SystemExit("Stage C23 audit evidence copies are missing")
            append_result(results, "candidate-evidence-copy", f"validated candidate and exact-rollback transaction retained non-authoritatively at {evidence_root}")
            append_result(results, "exact-rollback-close-c23", "typed C23 closure accepted only the adapter-generated 28-file exact-rollback transaction")
            if adapter.transaction_path is not None or not close_result.payload.transaction_path_absent or not close_result.payload.parents_restored or close_result.payload.installed_file_count != 28 or close_result.payload.payload_file_count != 27 or Path(close_result.payload.audit_evidence) != evidence_root:
                raise SystemExit("Stage C23 closure did not finish exact transaction cleanup")
            ...
            append_event(events, 60, "exact-rollback-transaction-closed", transaction.transaction.value)

            write_identity(evidence_root / "identity.tsv", transaction, lease.lease_id, invoking_user, managed_files_installed=True, filesystem_restored=True, services_restored=True)
            typed = (host_result, lock_result, acquire_result, create_result, filesystem_result, service_result, mixer_result, loopback_result, dac_result, stage_result, *validation_results, stop_result, release_dac_result, install_result, rollback_result, restore_result, dashboard_result, verify_result, abort_result, service_close_result, close_result)
            (evidence_root / "typed-operations.json").write_text(
                json.dumps({
                    ...
                    "stage_c22_manifest_sha256": STAGE_C22_EVIDENCE_MANIFEST_SHA256,
            ...
            append_result(results, "production-lock-released", "canonical production lock removed only after exact C23 closure")
            append_event(events, 70, "production-lock-released", lease.lease_id)
            write_report(evidence_root / "report.txt", evidence_root, transaction, len(blocked), len(approval_blocked))

        post_live = ProductionPrepareOnlyInspectorV7(ReadOnlyHostProductionAdapter(), package).inspect()
        ...
        for label, root in (("package", package_root), ("baseline", baseline_root), ("stage-c21", stage_c21_root), ("stage-c22", stage_c22_root)):
            if tree_fingerprint(root) != inputs[label]:
                raise SystemExit(f"{label} input changed during Stage C23")
        append_result(results, "input-integrity", "package, baseline, Stage C21 and Stage C22 evidence trees remained unchanged")
        _assert_regular_tree(evidence_root, "Stage C23 current-package managed-file evidence")
        ...
        append_result(results, "activation-interface", "absent; all 28 files were removed before reload, route selection, managed service startup, probes, approval or commit")
        write_evidence_manifest(evidence_root)
        observed = tuple(line.split("\t", 1)[0] for line in results.read_text(encoding="utf-8").splitlines()[1:])
        if observed != EXPECTED_CHECKS:
            raise SystemExit(f"unexpected Stage C23 result order: {observed}")
        completed = True
    ...
    if not completed:
        raise SystemExit("Stage C23 current-package exact-rollback rehearsal did not complete")
    print(
        f"""A Clockwork Plex Stage C23 current-package managed-file exact-rollback rehearsal passed.
...
All 28 accepted current-package files were atomically installed while the
application services and DAC were quiesced, then removed through the exact
inode ledger before any daemon reload or route selection. The accepted direct
appliance state was restored and persistent activation remains blocked.""".strip()
    )
    return 0
