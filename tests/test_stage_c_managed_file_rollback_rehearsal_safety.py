...
class StageCManagedFileRollbackRehearsalSafetyTests(unittest.TestCase):
    ...
    def test_rollback_is_armed_before_first_write(self) -> None:
        ...
        self.assertLess(create.index("self._arm_managed_rollback()"), create.index("os.mkdir("))
        self.assertLess(install.index("self._arm_managed_rollback()"), install.index("os.open(temporary"))
    ...
    def test_rollback_ledger_adopts_created_path_before_later_failure(self) -> None:
        ...
        self.assertLess(install.index("os.replace("), install.index("self._installed_files.append(record)"))
        self.assertLess(install.index("self._installed_files.append(record)"), install.index("os.fsync(parent_fd)"))
    ...
    def test_install_acceptance_is_stricter_than_rollback_identity(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(
                ManagedFileRollbackRehearsalAdapterV4._verify_rollback_identity(record),
                path,
            )
            with self.assertRaises(ManagedFileRollbackFailure):
                ManagedFileRollbackRehearsalAdapter._verify_installed_object(record)
            path.unlink()
    ...
    def test_failure_cleanup_rolls_files_back_before_services(self) -> None:
        ...
        self.assertLess(source.index("self._restore_managed_files_exact()"), source.index("super().__exit__"))
        self.assertIn("production lock and transaction are intentionally retained", source)
    ...
    def test_blocked_operations_are_confined_to_proof_and_post_install_gate(self) -> None:
        ...
        for name, locations in owners.items():
            expected = {"prove_blocked_operations", "main"} if name in {
                "reload_systemd",
                "select_split_bus_route",
    ...
    def test_wrapper_is_prepare_only_with_one_constrained_sudo(self) -> None:
        ...
        self.assertIn("STAGE-C18-MANAGED-FILES-EXACT-ROLLBACK", self.wrapper)
        self.assertIn("a-clockwork-plex-stage-c17-service-quiescence.3ySKhd", self.wrapper)
    ...
    def test_python_and_shell_syntax(self) -> None:
        for path in (
            ...
            V4,
            ENGINE,
            REPO_ROOT / "scripts/stage_c_transaction/production_adapter_lifecycle_v4.py",
        ):
