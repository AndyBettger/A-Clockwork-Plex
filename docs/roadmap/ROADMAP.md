...
- [ ] Final #86 acceptance: confirm the latest `develop` Actions run is green, pull the final head onto the development Pi, verify exact `GU30 7JS` now returns/selects a result, and verify the far-future **Unknown conditions** daily card no longer appears on the 16-day Weather forecast.
...
- [ ] **Friendly forecast-location entry — #86 in progress.** Town/city lookup is implemented and physically proven end-to-end on the development Pi. Full UK postcode lookup now has a Postcodes.io fallback when Open-Meteo has no match, and the two-stage Weather forecast renderer suppresses unusable daily entries whose conditions are unknown. Exact coordinate entry remains the advanced/fallback path. Final Actions plus one combined physical postcode/16-day forecast re-check remain before closure.
