...
- [x] First known-source playback case: with a real **24/96 FLAC**, Plexamp direct-played the file, created its source stream and internal mixer at **96 kHz**, and explicitly preferred/best-selected **96 kHz** for device 9 (`A Clockwork Plex - Plexamp`), but that managed device opened at **44.1 kHz** and every downstream ACP/Camilla/DAC point remained **S16_LE / 44.1 kHz**. The fixed ACP output-device chain is therefore the measured 24/96 rate bottleneck; Plexamp's decoder/mixer is not.
- [ ] Finish the known-source matrix with **16/44.1, 24/48 and 24/192** material before selecting the first higher-resolution managed-bus rehearsal.
...
- [ ] Physical capability audit with known 16/44.1, 24/48, 24/96 and 24/192 Plex files, recording source/Plexamp, processing-bus and final-DAC format/rate separately. The 24/96 case is complete and localises the rate collapse to the managed output-device boundary: Plexamp source/mixer/preferred device rate are 96 kHz, while `A Clockwork Plex - Plexamp` opens at 44.1 kHz and the downstream graph remains S16/44.1.
