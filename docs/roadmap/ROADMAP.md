...
- [x] That pass identified the remaining Settings usability refinement: the large feed-editor cards are sensible for editing but poor for long-distance ordering, and the Add BBC feed control/new-row location were not obvious. The branch now separates **News feeds** (rename/add/check/remove only) from a compact **News → Feed order** page with a touch drag grip and Enabled/Disabled control per row; Add BBC feed uses a normal button footprint and scrolls/highlights the newly created editor.
...
- [ ] Commissioned 1280×720 recheck confirms the Add BBC feed auto-scroll/highlight, centred grip geometry and removal of duplicate Enabled/Move controls from News feeds, then completes custom-feed story QR, non-BBC rejection, cache-source correctness and final touch/layout usability.
