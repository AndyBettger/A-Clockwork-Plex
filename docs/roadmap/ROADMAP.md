...
- [ ] **Release screenshots / visual first-use guide.** The owner supplied a coherent 1280×720 appliance screenshot set on 22 August 2026 covering daytime Clock, night Clock, active alarm, Alarm Settings, Master EQ Settings, Weather Settings, forecast/current/wind-rain Weather views, AirPlay Ready/Now Playing and Plexamp Now Playing. Curate only privacy-safe frames for the public docs; the Weather station-status frame contains a live station identifier and is intentionally excluded. A refreshed About screenshot should be captured after the About/version work. A physical hardware hero photograph and finished NFC sleeve photograph remain welcome later but are **not release blockers**.
