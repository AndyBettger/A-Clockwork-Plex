...
- Rainfall-history refresh ownership is serialized across background, manual/Test Connection and Settings-triggered refreshes. The dedicated refresh mutex covers the complete cache read/fetch/merge/write transaction while the ordinary status/snapshot lock remains independent, preventing concurrent owners from colliding on `weather-rainfall-history.json.tmp` without holding UI status reads across network calls.
...
- The earlier spare/test SD developed intermittent read-only behaviour and is not valid final evidence. The replacement spare SD has now completed the final physical clean-room sequence: Plexamp/EQ commissioning, checkpoint #58 Weather rainfall-history retest, AirPlay, NFC, real scheduled-alarm/fade/safety proof, representative commissioned reboot, both formal verifier sets, checkpoint #63 repeat public `setup.sh` idempotence with WU preserved, and checkpoint #64 empty `git status --porcelain` clean-checkout proof. No further physical clean-room gate is outstanding; release hygiene/final validation remain.
...
### Phase 7 — full appliance installer integration — **In progress: replacement-SD physical clean-room acceptance is complete through checkpoint #64; deliberate repository/release hygiene, final validation and explicit owner approval remain**
...
- [x] **Replacement-SD focused retest:** after checkpoint #58 serialized the complete WU rainfall-history refresh transaction, the replacement card physically exercised Current year, Test Connection, Last 7 days and Current year again without reproducing the atomic `.tmp` rename failure. The dashboard service remained active and focused journal output contained only successful rainfall API `200` requests. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
...
- [x] Replace the main time-like free-text Settings fields with bounded preset selectors where a small, useful set of values is appropriate: Dashboard idle timeout, Weather Online forecast Refresh interval and AirPlay pause hold. Existing non-preset saved values remain representable as a temporary `Current (...)` option rather than being silently changed.
- [x] Give the remaining Weather numeric inputs explicit typed bounds/steps in the initial pass: Ecowitt freshness, WU refresh/stale/request-timeout intervals and forecast latitude/longitude.
- [x] Harden Weather backend validation so out-of-range WU numeric values fail closed instead of relying on browser controls alone.
- [x] Add late `settings-theme-closure.css` semantic ownership for residual non-semantic info/warning controls, badges, form focus/range/check/tab/helper paint and Settings maintenance surfaces; regression coverage rejects reintroduction of fixed cyan presentation paint.
- [x] Exact initial code/test head `8f76a8f33e770bf8dda89cd43f95cd4db2782abf` passed `pytest` run `32414179057`: compile/JavaScript/page/shell steps PASS and **1,751/1,751 unit tests PASS**.
- [x] **Physical focused re-check, 20 August — persistence/palette PASS:** changed Settings values save and survive reload; a full-interface sweep in Crimson Glow and Green Phosphor found no residual cyan leakage; and no 1024×600 clipping/layout defect was reported.
- [x] **Physical #54 preset re-check, 20 August — PASS:** General > Idle timeout renders as a true dropdown; both reported Weather Refresh interval controls render as dropdowns; AirPlay > Handoff renders as a dropdown with **7 minutes** as the maximum; and changed values persist correctly. These controls do not require another physical pass unless a later regression is noticed.
- [x] **AirPlay pause-hold contract:** remove obsolete long hold choices and cap UI, application runtime and `scripts/set-airplay-hold-seconds.py` consistently at **7 minutes / 420 seconds** (minimum 15 seconds). The Settings numeric-control asset was cache-busted to `numeric-controls-v2` so kiosk Chromium did not retain the superseded spinner implementation.
- [x] Exact #54 code/test head `039fd621306b5f247ee6df75da77f6f3df0ed9ae` passed Tests #3979 / run `32422530745`: Python compile PASS, JavaScript/page wiring PASS, shell syntax PASS and **1,752/1,752 unit tests PASS**.
- [x] **Weather timing-control closure source:** Ecowitt Freshness window plus WU Poll interval, Stale after and Request timeout are now true touch dropdowns rather than native `type=number` spinner fields. Saved non-preset timing values remain representable as temporary custom choices. Forecast latitude/longitude remain deliberate exact-entry controls, but are forced to text/custom-decimal-keyboard ownership so Chromium no longer paints native up/down spinner arrows.
- [x] The numeric-control asset is cache-busted again to `numeric-controls-v3`; regression coverage now fails if those four Weather timing controls return to native number inputs or if latitude/longitude regain native spinner ownership.
- [x] Exact #55 source/test head `e786c1c653083f6de9c0b1daf1a87ab975b55f25` passed Tests #3984 / run `32425069018`: Python compile PASS, JavaScript/page wiring PASS, shell syntax PASS and **1,753/1,753 unit tests PASS**.
...
- [x] The earlier test SD's intermittent read-only failure has been retired from final evidence; the replacement spare SD is the accepted release-candidate clean-room card.
- [x] Main presentation, palette closure and Settings touch-control ergonomics are accepted through checkpoint #55.
- [x] **Final replacement-SD position:** the clean-room run passed initial source/checkout identity, CamillaDSP identity, Plexamp GUI/output commissioning, Plexamp/EQ, checkpoint #58 WU rainfall-history retest, AirPlay takeover/EQ/disconnect, NFC album-tag/debounce, real scheduled-alarm fade/Snooze/re-ring/Dismiss/Music-Master-independence, representative commissioned reboot, the first formal verifier set, checkpoint #63 plain public `bash setup.sh` repeat-install plus the second formal verifier set while preserving Weather Underground and working EQ/Plexamp audio, and checkpoint #64 empty `git status --porcelain` proof on the exact physically tested runtime head. Physical replacement-SD clean-room acceptance is complete.
- [x] On the replacement spare SD, complete the final `docs/INSTALL.md` / `docs/fresh-appliance-acceptance-runbook.md` functional run through NFC, alarm/fade and representative reboot.
- [x] After the representative post-commissioning reboot on that replacement card, run `verify-fresh-bootstrap.sh`, `verify-appliance.sh --audio eq --weather-observations weather-underground` and `scripts/audio/verify-audio.sh`, and preserve the non-secret outputs as final evidence. Result: `FRESH_BOOTSTRAP_VERIFY=PASS`, `APPLIANCE_VERIFY=PASS`, audio verifier PASS, zero failures/warnings in the two structured verifiers.
- [x] Rerun final public `setup.sh` on that installed replacement card to prove idempotence with no renewed claim/reboot checkpoint or ownership drift, then rerun the three formal verifiers. Result: after the source fix for commissioned-Weather preservation, plain `bash setup.sh` retained `weather-underground`, used `preserve-commissioned-profile`, committed successfully with no claim or exit-75 reboot checkpoint, and the repeat bootstrap/appliance/audio verifiers all passed; real Plexamp audio and EQ remained working. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- [x] Confirm normal operation does not dirty tracked files (`git status --porcelain`). Result: exact physically tested runtime head `215bced...` produced no output after the repeat setup/verifier/normal-operation sequence; `40-git-status.txt` is intentionally empty. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- [x] Finalize the clean-room/reboot/repeat/clean-checkout evidence documents around the completed physical gate. Release-hygiene/final-validation work remains separate and must still complete before Phase 7 closes.

#### Remaining Phase 7 execution sequence — authoritative order

1. [x] **Finish tracked runtime/generated-state + `.gitignore` audit.** Known runtime/cache/log/build/editor artefacts are ignored; committed-source checks use Git tracking rather than rejecting legitimate ignored runtime files produced by tests; runtime-generated `*.json.tmp` state is covered. Release-hygiene regression remains green at code head `ae2497450b5b9d106c2eb4d86301bd1bc32c455b`.
2. [x] **Audit and pin fresh-install repository dependencies.** `installer/repository-dependencies.txt` now pins the supported dependency closure required by `setup.sh` / `appliance-installer.sh`, including transitive installer libraries, EQ repair/profile/template assets, NFC requirements/checker/runtime, `config.example.json` and dashboard unit source. Both public and lower-level paths fail closed early on an incomplete repository payload; Tests #4012 / run `32430838605` passed **1,762/1,762** unit tests plus compile/JavaScript/page/shell checks.
3. [x] **Refresh PR #2 description without changing its Draft/open state.** On 21 August the stale Stage-C/future-EQ narrative and obsolete 490-test count were replaced with the integrated release-candidate scope. The description now tracks the replacement-SD rainfall-history race/fix, checkpoint #63 repeat-install preservation proof and completed clean-room position. PR #2 remains Draft, open and unmerged.
4. [ ] **Complete safe repository/release hygiene.** The replacement-SD clean-room proof is now complete, so the previously deferred inventory/consolidation work may proceed deliberately: remove only clearly obsolete probes/scaffolding/generated leftovers, review large historical-doc provenance rather than blindly deleting it, proofread active operator docs, and remove temporary development refs before merge.
5. [x] **Finish the replacement-SD release gate.** The active run cleared all major functional tests, representative reboot, both formal-verifier sets, repeat public `setup.sh` idempotence with commissioned WU preserved, and the final empty `git status --porcelain` clean-checkout proof. No additional physical clean-room gate is currently outstanding.
6. [ ] **Rerun final dependency/tracked-file audit and complete validation after cleanup**, then obtain explicit owner approval before PR #2 can leave Draft or merge.

#### Final repository/release hygiene — physical clean-room boundary proven; deliberate cleanup remains
...
- [x] Known runtime/generated-state and `.gitignore` coverage audited and regression-protected; the replacement-SD final clean-checkout proof is now physically PASS.
- [x] Initial fresh-install dependency closure pinned in `installer/repository-dependencies.txt` and enforced by both supported installer entry paths.
- [x] PR #2 description refreshed for the current release candidate while preserving Draft/open/unmerged state.
...
- [x] Confirm a normally operating replacement-SD appliance leaves the checkout clean (`git status --porcelain`). Checkpoint #64: no output on exact physically tested runtime head `215bced...`; evidence file `40-git-status.txt` is intentionally empty.
...
- [ ] Rerun the final tracked-file/install-dependency audit after safe cleanup so every file required by `setup.sh`/`appliance-installer.sh` is still present and no retained file survives merely because it was used by an obsolete experiment.
- [ ] Run the complete validation suite after cleanup. PR #2 remains Draft until release hygiene, final validation and explicit owner approval are complete.

**Phase 7 exit condition:** final V3/theme/AirPlay/Settings/Clock/navigation presentation is accepted; the focused post-presentation Settings-control/palette re-check is complete; the replacement spare-SD appliance has passed functional/formal reboot, public `setup.sh` repeat-install, both formal-verifier sets and final clean-checkout proof; the repository must now complete deliberate release hygiene and final validation, and explicit owner approval is required. PR #2 remains Draft throughout.
...
- **#52 — managed WU credential presence-only appliance verification — PASS (source/CI + replacement-SD physical).** The root-owned weather-secret helper exposes restricted `status` that returns only `WEATHER_SECRET_CONFIGURED=0|1`; production `verify-appliance.sh` uses that managed status path when no deliberate key-file/environment override is supplied. Alternate-root application fixtures remain compatible, while helper/sudoers artifacts are still validated whenever either is staged. Exact code/test head `4893bdc1b0061ad0f918f9f075a70ba66cc12c52` passed Tests #3931 / run `32405435882`: compile PASS, JavaScript/page/shell PASS, **1,742/1,742 unit tests PASS**. **Physical 21 August:** the replacement-SD post-reboot `verify-appliance.sh --audio eq --weather-observations weather-underground` run confirmed the managed WU credential through the presence-only helper with no key disclosure, zero failures/warnings and `APPLIANCE_VERIFY=PASS`.
- **#53 — safer Settings numeric controls + semantic palette closure — PARTIAL PHYSICAL PASS; touch-control delivery superseded by #54.** Dashboard idle timeout, Weather auto-refresh and AirPlay pause hold gained preset-selector behaviour while remaining Weather numeric fields gained explicit typed bounds and backend validation; `settings-theme-closure.css` closes residual non-semantic palette leakage. Initial code/test head `8f76a8f33e770bf8dda89cd43f95cd4db2782abf` passed run `32414179057` with **1,751/1,751 unit tests PASS**. **Physical 20 August:** persistence passed, the entire interface was swept in Crimson Glow and Green Phosphor with no cyan found, and no 1024×600 clipping/layout defect was reported. The Pi still showed spinner-style controls with tiny arrows/keyboard, so touch ergonomics did not pass and were carried into #54.
- **#54 — touch-first Settings presets + seven-minute AirPlay hold closure — PASS (source/CI + physical).** The preset-control asset was cache-busted to `numeric-controls-v2`; General > Idle timeout and the reported Weather Refresh interval controls rendered as true dropdowns, AirPlay > Handoff rendered as a dropdown with a **7-minute maximum**, and saved values persisted correctly on the real appliance. AirPlay pause hold is consistently bounded to **15–420 seconds** across Settings, application runtime and the standalone setter helper, with obsolete >7-minute choices removed. Exact code/test head `039fd621306b5f247ee6df75da77f6f3df0ed9ae` passed Tests #3979 / run `32422530745`: Python compile PASS, JavaScript/page wiring PASS, shell syntax PASS and **1,752/1,752 unit tests PASS**. The same physical pass exposed additional native Weather provider timing spinners outside #54's narrow preset set; those are carried into #55.
...
- **#56 — release-hygiene runtime-state audit + fresh-install repository dependency closure — PASS (source/CI).** Known runtime/generated/tooling state is ignored and tracked-source regression now tests the Git tracking contract rather than failing because another test created an ignored runtime file. `installer/repository-dependencies.txt` pins the supported source closure and both `setup.sh` and direct `appliance-installer.sh` fail closed before installation if a required source is missing; transitive EQ repair, installer libraries, NFC runtime/checker/requirements, dashboard unit, `config.example.json` and EQ profile/template assets are explicitly protected. Exact code head `ae2497450b5b9d106c2eb4d86301bd1bc32c455b` passed Tests #4012 / run `32430838605`: Python compile PASS, JavaScript/page wiring PASS, shell syntax PASS and **1,762/1,762 unit tests PASS**.
- **#57 — PR #2 release-candidate description refresh — PASS (metadata; Draft preserved).** On 21 August the PR body was replaced with the actual integrated appliance scope, current installer/runtime contracts, current CI baseline and remaining replacement-SD/release gates. It has since been refreshed through the completed checkpoint #63 repeat-install gate; GitHub confirms `state=open`, `draft=true`, `merged=false`; no readiness or merge action was performed.
- **#58 — WU rainfall-history refresh serialization — PASS (source/CI + replacement-SD physical).** The replacement-SD clean-room Weather commissioning run exposed concurrent rainfall refresh owners sharing the atomic `weather-rainfall-history.json.tmp` write path. `WeatherRainfallHistoryService` now uses a dedicated refresh mutex around the complete load/fetch/merge/save transaction while retaining the separate status/snapshot lock, so slow WU requests do not hold normal UI status access. Regression `test_overlapping_refreshes_are_serialized_before_cache_update` deliberately blocks the first Last-7-days fetch, starts a second refresh and proves it cannot enter the fetch/cache transaction concurrently; after release the second refresh reuses the populated cache. Behaviour/test head `2e8ca2b41701cae054be19ade27fb9328c34f635` passed Tests #4031 / run `32519326364` with **1,763/1,763 unit tests PASS**. CI-hardening head `0723da83a4a08a332e7eb42bf2016b564c2e72d5` additionally places `app/weather_rainfall_history.py` in the explicit `py_compile` set and passed Tests #4035 / run `32520278486`: Python compile PASS, JavaScript/page wiring PASS, shell syntax PASS and **1,763/1,763 unit tests PASS**. No Ruff configuration/check exists in the repository, so no Ruff result is claimed. **Physical 21 August:** the replacement card advanced cleanly to `dcb4433e8c72350f4c11de2f643f83a4ffa6a1a9`, the dashboard service returned active, Current year/Test Connection/Last 7 days/Current year all completed normally, the prior rename collision did not recur, and focused journal output showed only successful rainfall API `200` requests. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- **#59 — replacement-SD AirPlay handoff/EQ proof — PASS (physical).** With Plexamp playing on the clean-room appliance, selecting the A Clockwork Plex AirPlay receiver paused Plexamp and moved the dashboard to AirPlay; AirPlay audio was clean, Bass/EQ changes were audible, Bypass EQ and Return to normal behaved correctly, disconnect returned the dashboard to Clock, and Plexamp could be manually resumed. Before/after service evidence was identical: `plexamp.service` PID `1909`, `shairport-sync.service` PID `5082`, `a-clockwork-plex-camilladsp.service` PID `944`, all `ActiveState=active`, all `NRestarts=0`. The ordinary handoff therefore caused no Plexamp, Shairport Sync or CamillaDSP restart. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- **#60 — replacement-SD NFC album-tag/debounce proof — PASS (physical).** A known-good album tag switched the dashboard to Plexamp and started the correct album. `nfc-listener.service` remained active on PID `1910` with `NRestarts=0`. A same-UID presentation within roughly two seconds produced no second UID/playback action, while a presentation after the five-second debounce window restarted playback as expected. A malformed/truncated boundary read was safely rejected. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- **#61 — replacement-SD real scheduled-alarm fade/safety proof — PASS (physical).** With Plexamp playing, a real scheduled alarm paused Plexamp, started quietly and audibly faded over 20 seconds, Snooze stopped it without auto-resuming Plexamp, the one-minute re-ring started a fresh quiet fade, and Dismiss stopped it without auto-resume; Plexamp could then be manually restarted. The shared alarm mixer was independently confirmed at 25% (`-12.2 dB`) while Music Master was 66% (`-3.6 dB`), and a second physical occurrence remained audible with Music Master temporarily at 0%, proving the alarm lane bypasses Music Master while retaining the dedicated Maximum Alarm Volume ceiling. Dashboard, Plexamp and canonical CamillaDSP remained active with zero restart counters. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- **#62 — replacement-SD representative reboot + first formal verifiers — PASS (physical).** The commissioned appliance rebooted cleanly on runtime/source head `dcb4433e8c72350f4c11de2f643f83a4ffa6a1a9`; dashboard, Plexamp, NFC listener, Shairport Sync and canonical CamillaDSP all returned active/enabled, and real Plexamp playback worked. `verify-fresh-bootstrap.sh` returned zero failures/warnings and `FRESH_BOOTSTRAP_VERIFY=PASS`; `verify-appliance.sh --audio eq --weather-observations weather-underground` returned zero failures/warnings and `APPLIANCE_VERIFY=PASS`; `scripts/audio/verify-audio.sh` reported EQ-capable audio verification passed. Evidence outputs are preserved as `20-post-reboot-bootstrap-verifier.txt`, `21-post-reboot-appliance-verifier.txt` and `22-post-reboot-audio-verifier.txt` under the clean-room evidence directory. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- **#63 — repeat public setup commissioned-Weather idempotence — PASS (source/CI + replacement-SD physical).** The first physical repeat exposed a real regression: plain public `setup.sh` treated its fresh-install Ecowitt default as an operator choice and could replace an already-commissioned WU observation profile. Source head `215bcedb43369844b5968ae24a7169e49636ef99` adds an internal preserve-commissioned-profile path when an existing appliance is rerun without an explicit Weather option, resolves the live provider from `config.json`, verifies preserved WU credential presence through the restricted boolean-only helper, skips Weather mutation, and still runs the normal whole-appliance verifier; explicit Weather selections retain their existing convergence semantics. Dedicated `tests/test_weather_repeat_preservation.py` regression coverage and Tests #4063 passed. **Physical retest, 22 August:** after a clean fast-forward to exact head `215bced...`, plain `bash setup.sh` reported `Weather observations: weather-underground` and `Weather mutation: preserve-commissioned-profile`, preflights returned zero failures/warnings, the application transaction printed `Preserving commissioned Weather configuration and managed credential`, final `APPLIANCE_VERIFY=PASS`, `APPLICATION_TRANSACTION=COMMITTED` and `ROOT_INSTALL=COMMITTED`, and setup completed without a claim checkpoint or exit-75 reboot-required stop. The post-repeat evidence files `33-repeat-bootstrap-verifier.txt`, `34-repeat-appliance-verifier.txt` and `35-repeat-audio-verifier.txt` then returned `FRESH_BOOTSTRAP_VERIFY=PASS`, `APPLIANCE_VERIFY=PASS` and EQ-capable audio verification passed; the two structured verifiers again had zero failures/warnings. Real Plexamp audio remained audible, EQ remained active and Weather Underground remained the observation source. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
- **#64 — replacement-SD final clean-checkout proof — PASS (physical).** After checkpoint #63's successful plain public repeat setup, second formal verifier set and normal operation, the replacement appliance deliberately remained on exact physically tested runtime/source head `215bcedb43369844b5968ae24a7169e49636ef99` before the evidence-only documentation commits were pulled. `git status --porcelain | tee "$EVIDENCE/40-git-status.txt"` produced no output; `40-git-status.txt` is intentionally empty. This proves normal commissioned operation, Weather/cache activity, Plexamp/NFC, managed EQ/audio, installer rerun and verifier execution do not dirty tracked source. The physical replacement-SD clean-room gate is complete. Evidence: `docs/final-clean-room-physical-progress-2026-08-21.md`.
