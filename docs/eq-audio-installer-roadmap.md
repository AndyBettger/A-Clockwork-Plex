...
### Phase 7 — full appliance installer integration — **In progress: functional/audio/Weather/annunciator/V3, daytime palettes, AirPlay long-form presentation, night-preview timing and the main Settings/Clock theme-control pass are physically accepted; residual Alarm/Hardware/navigation/theme-state chrome, replacement-SD clean-room install, formal verifiers and release hygiene remain**
...
#### Daytime theme — physical PASS with residual Alarm/Hardware/navigation/theme-state chrome follow-up in progress
...
- [ ] **Residual physical follow-up:** confirm the nested Alarm editor surfaces/Add-another-alarm control, Audio Hardware maintenance note, EQ subtitle removal, bottom-navigation inactive/active colour states, non-Classic EQ bypass active/bypassed states and legacy Settings subsection headings are all correct on the real display.
...
#### Final presentation closure — main Settings/Clock pass physically accepted; residual Alarm/Hardware/navigation/theme-state physical pending
...
- [x] Residual non-Classic Alarm editor surfaces now hand border/background ownership to the active palette: Alarm/editor card, Repeat schedule, Snooze behaviour, Low-level preview, time picker/preview row and the dotted `+ Add another alarm` control no longer intentionally use legacy cyan presentation paint.
- [x] The read-only Audio > Hardware maintenance note (`The physical output is ...`) now uses theme-control border/fill rather than its legacy cyan treatment.
- [x] The compact Master bus EQ subtitle `Plexamp + AirPlay · music only` is removed from presentation; the surrounding mixer already establishes the source boundary.
- [x] Bottom navigation now has its own final theme authority: inactive buttons use neutral palette control paint, current-page `.is-active` uses the saturated theme fill, and the broad Settings action-button rule is scoped to `.settings-page` so it cannot flatten the Settings current-page state or the Audio drawer's `.is-active` state after the nav is moved under `<body>`.
- [x] Exact final source/test head `94eed0bc7d389e7eb509842320cffb48563a1ac2` passed Tests #3901 / run `32333683584`: compile PASS, JavaScript/page/shell PASS, **1,730/1,730 unit tests PASS**. Tests #3899 had exposed only a brittle stylesheet-order assertion after the cache-version change; the runtime/source changes were not the failure and the assertion was corrected before #3901.
- [x] Non-Classic Master EQ bypass state now has final theme ownership in both the EQ base layer and late theme follow-up: an active/pressed Bypass EQ button uses the selected theme's saturated accent and contrast text, while the bypassed health/status treatment follows the selected palette. The pre-existing Classic Dark amber bypass baseline remains explicitly regression-pinned and unchanged.
- [x] Legacy `.acp-setting-section > h3` Settings headings now use the selected palette accent and control-border colour instead of carrying the older cyan heading paint into non-Classic themes.
- [x] Exact current code/test head `51fdaae1d1ce271b9202b71637ffa52930c28172` passed Tests #3923 / run `32364917025`: compile PASS, JavaScript/page/shell PASS, **1,735/1,735 unit tests PASS**. The preceding source head `719e613e8faa25c0e1011ac072f7dc2613b8aa86` passed CodeQL run `32341515508`; its Tests #3915 / run `32341515510` reached the unit suite after compile/JS/shell success and failed only three stale cache-version assertions (`v3`/`v5` versus the live `v7` asset), not runtime/source behaviour. Those regressions were aligned and expanded to cover the final EQ/heading contracts before #3923.
- [ ] **Physical:** Settings > Alarms > Alarm schedule — confirm Alarm, Repeat schedule, Snooze behaviour, Low-level preview and `+ Add another alarm` no longer show a cyan-tinted outline/background in non-Classic themes.
- [ ] **Physical:** Settings > Audio > Hardware — confirm the bottom physical-output maintenance note now follows the current theme rather than cyan.
- [ ] **Physical:** Audio page — confirm the small `Plexamp + AirPlay · music only` copy is absent from the Master bus EQ box.
- [ ] **Physical:** bottom navigation — confirm inactive buttons use the current palette rather than cyan; current Clock/Weather/AirPlay/Settings pages fill correctly; Settings is visibly active on Settings; and opening Audio lights Audio whether opened from Settings or another dashboard page.
- [ ] **Physical:** Audio > Master Equalizer — with a non-Classic theme selected, confirm Bypass EQ active/pressed and the bypassed health state use that theme rather than the old cyan/amber presentation; then confirm Classic Dark still uses its established amber bypass state.
- [ ] **Physical:** any Settings page using the legacy `.acp-setting-section` wrapper — confirm subsection headings and underline/border accents follow the selected non-Classic theme rather than cyan.
...
- [ ] After the remaining focused Alarm/Hardware/navigation/theme-state presentation follow-up is visually accepted on usable media, run bootstrap/application/audio verifiers after a representative final reboot and commit the evidence.
