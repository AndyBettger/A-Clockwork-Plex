...
- The earlier spare/test SD developed intermittent read-only behaviour and is not valid final evidence. A replacement spare SD is now the active clean-room target; the current run has passed Plexamp/EQ commissioning, checkpoint #58 Weather rainfall-history retest, AirPlay, NFC, the real scheduled-alarm/fade/safety proof, a representative commissioned reboot, the first formal verifier set, and checkpoint #63 repeat public `setup.sh` plus repeat-verifier idempotence proof. The next physical gate is the final clean-checkout proof.
...
### Phase 7 — full appliance installer integration — **In progress: all major replacement-SD functional gates, representative reboot, both formal-verifier sets and repeat public setup idempotence are physically complete; clean-checkout proof and release hygiene remain**
...
- [x] The earlier test SD's intermittent read-only failure has been retired from final evidence; the replacement spare SD is now the active release-candidate clean-room card.
- [x] Main presentation, palette closure and Settings touch-control ergonomics are accepted through checkpoint #55. Final release evidence is now being collected on the replacement spare SD.
- [x] **Current replacement-SD position:** the clean-room run has passed initial source/checkout identity, CamillaDSP identity, Plexamp GUI/output commissioning, Plexamp/EQ, checkpoint #58 WU rainfall-history retest, AirPlay takeover/EQ/disconnect, NFC album-tag/debounce, real scheduled-alarm fade/Snooze/re-ring/Dismiss/Music-Master-independence, the representative commissioned reboot, the first formal verifier set, and checkpoint #63 plain public `bash setup.sh` repeat-install plus the second formal verifier set while preserving Weather Underground and working EQ/Plexamp audio. The next gate is the final `git status --porcelain` clean-checkout proof.
...
3. [x] **Refresh PR #2 description without changing its Draft/open state.** On 21 August the stale Stage-C/future-EQ narrative and obsolete 490-test count were replaced with the integrated release-candidate scope. The description now also tracks the replacement-SD rainfall-history race/fix and latest validated source evidence. PR #2 remains Draft, open and unmerged.
...
5. [ ] **Finish the replacement-SD release gate.** The active run has cleared all major functional tests, the representative reboot, both formal-verifier sets and the repeat public `setup.sh` idempotence proof with commissioned WU preserved. Remaining physical proof is the clean `git status --porcelain` check, followed by final evidence/documentation, final full validation and explicit owner approval before PR #2 can leave Draft or merge.
...
- [x] Known runtime/generated-state and `.gitignore` coverage audited and regression-protected; final clean-checkout proof remains part of the replacement-SD gate.
...
- [ ] Confirm a normally operating replacement-SD appliance leaves the checkout clean (`git status --porcelain`).
...
**Phase 7 exit condition:** final V3/theme/AirPlay/Settings/Clock/navigation presentation is accepted; the focused post-presentation Settings-control/palette re-check is complete; the replacement spare-SD appliance passes functional/formal reboot plus the final public `setup.sh` clean-room install and repeat-install; `verify-fresh-bootstrap.sh`, `verify-appliance.sh` and `scripts/audio/verify-audio.sh` are green where applicable; the repository is clean and documented; and explicit owner approval is given. PR #2 remains Draft throughout.
