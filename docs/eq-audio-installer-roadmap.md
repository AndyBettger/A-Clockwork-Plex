...
### Phase 7 — full appliance installer integration — **In progress: final Weather/alarm physical pass accepted on the current Pi except deferred WU-only indoor expiry; Clock annunciator first physical pass exposed a small activation/artwork follow-up whose corrected source/CI is green; second display check, final public clean-room install, verifiers and release hygiene remain**
...
#### Final Clock/navigation polish — alarm annunciator follow-up source/CI PASS; second physical display check pending
...
- [ ] **14-segment artwork refinement:** revisit the underlying SVG segment path geometry/shape artwork after the functional release polish. Preserve the accepted mapping semantics (numeric `0` slashed, capital `O` unslashed, `W` without bottom segment) while allowing the actual segment shapes/proportions to be redrawn and re-tested visually.
...
- [x] **Source/CI follow-up:** the activation issue was a real producer/consumer contract bug: scheduler occurrences publish `scheduled_for`, while the first Clock client read nonexistent `when`. The client now consumes `scheduled_for` with `when` retained only as a compatibility fallback; regression coverage explicitly pins the scheduler producer field to the Clock consumer. The glyph moves from -10° to +20° (about 30° clockwise), gains the missing fourth tick, and the active state is raised to 0.96 opacity with a stronger `currentColor` glow so Classic/Astronomy tinting remains authoritative. Corrected head `15b699724ba6feeda72cfddc0e008587aaea21b1` passed **Tests #3701 / run `32192606982`**.
- [ ] **Physical follow-up:** on the Touch Display 2, confirm the +20° orientation/four ticks and that the current within-12-hours morning alarms now produce an unmistakably lit but still LCD-like bell. If convenient, switch once to **Any future alarm** to confirm the persisted mode changes the illumination rule; tune brightness only if the corrected active state is still visually wrong.
...
- [ ] After Clock-annunciator physical acceptance and any final display tuning, wipe the spare SD once more and perform the final `INSTALL.md` clean-room run using **`setup.sh`** from the first public command through claim, Plexamp GUI commissioning, WU configuration, playback/EQ/AirPlay/NFC/alarm and reboot.
...
- **#40 — alarm annunciator activation/artwork correction — PASS (source/CI; second physical pass pending).** Scheduler occurrences authoritatively publish `scheduled_for`; the first Clock client incorrectly consumed `when`, leaving the bell inactive. The corrected client consumes `scheduled_for` (with compatibility fallback only), the producer/consumer contract is regression-pinned, the glyph is +20° with four ring ticks, and the active state uses 0.96 opacity plus a stronger theme-aware `currentColor` glow. Head `15b699724ba6feeda72cfddc0e008587aaea21b1` passed **Tests #3701 / run `32192606982`**. The next gate is the narrow Touch Display 2 recheck of active brightness/orientation and mode behavior.
