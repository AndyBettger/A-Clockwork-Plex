...
### Phase 7 — full appliance installer integration — **In progress: functional/audio/Weather/annunciator/V3 and main presentation are physically accepted; focused Settings-control/palette re-check, replacement-SD clean-room install, formal verifiers and release hygiene remain**
...
#### Settings numeric-control + semantic palette closure follow-up — persistence/palette physical PASS; touch-dropdown re-check pending

- [x] Replace the main time-like free-text Settings fields with bounded preset selectors where a small, useful set of values is appropriate: Dashboard idle timeout, Weather auto-refresh and AirPlay pause hold. Existing non-preset saved values remain representable as a temporary `Current (...)` option rather than being silently changed.
- [x] Give remaining Weather numeric inputs explicit typed bounds/steps: Ecowitt freshness, WU refresh/stale/request-timeout intervals and WU latitude/longitude.
...
- [ ] **Physical touch-control re-check:** the first Pi pass still rendered spinner-style numeric controls with tiny up/down buttons and summoned the touch keyboard, which is not accepted. The follow-up now forces/cache-busts true dropdown delivery for Dashboard idle timeout, Weather auto-refresh and AirPlay pause hold. Confirm all three are actual touch dropdowns, do not summon the keyboard, and still persist after reload.
- [x] **AirPlay pause-hold contract:** remove obsolete long hold choices and cap UI, application runtime and `scripts/set-airplay-hold-seconds.py` consistently at **7 minutes / 420 seconds** (minimum 15 seconds). The Settings numeric-control asset is cache-busted to `numeric-controls-v2` so kiosk Chromium does not retain the superseded spinner implementation.
- [x] Exact follow-up code/test head `039fd621306b5f247ee6df75da77f6f3df0ed9ae` passed Tests #3979 / run `32422530745`: Python compile PASS, JavaScript/page wiring PASS, shell syntax PASS and **1,752/1,752 unit tests PASS**.
...
- [ ] Main presentation and palette closure are accepted on the current appliance; one narrow Settings touch-dropdown ergonomics re-check remains. Final release evidence still waits for the replacement spare SD because the present test card is not trustworthy clean-room media.
