...
### Phase 7 — full appliance installer integration — **In progress: final Weather/alarm physical pass accepted on the current Pi except deferred WU-only indoor expiry; Clock annunciator source/CI is green with physical display acceptance pending; final public clean-room install, verifiers and release hygiene remain**
...
#### Final Clock/navigation polish — alarm annunciator source/CI PASS; physical display check pending
...
- [ ] **Physical:** confirm the bell's position/size, faint unlit appearance and restrained lit glow on the Touch Display 2; confirm the default 12-hour mode responds to the real scheduler next occurrence, then switch once to Any future alarm and confirm the setting changes the illumination rule.
...
- **#39 — LCD-style Clock alarm annunciator — PASS (source/CI; physical pending).** Clock now includes a passive top-right bell annunciator with faint unlit and restrained lit states. The existing Clock `/api/status` refresh drives it from authoritative `alarm_scheduler.next_occurrence`; Settings → Display → Clock exposes **Next alarm within 12 hours** (default) and **Any future alarm**. The bell lives inside the Clock hero so established night treatment/burn-in authority applies. Fresh config now also agrees with the 10% alarm fade-start default. CI was hardened to compile the production promoted Settings modules and syntax-check both changed Clock clients; head `e687d14de733161645a9a37dde6936ba4e478fe0` passed **Tests #3687 / run `32182835949`**. Physical Touch Display 2 position/brightness/mode behavior is the next gate.
