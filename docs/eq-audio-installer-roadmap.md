...
- The earlier spare/test SD developed intermittent read-only behaviour and is not valid final evidence. A replacement spare SD is now the active clean-room target; the current run has reached Weather commissioning and all source changes remain committed to GitHub so the appliance remains reconstructable.
...
- [ ] **Replacement-SD focused retest:** the clean-room run exposed a concurrent atomic-cache rename collision while WU rainfall history was being commissioned. Source/test checkpoint #58 serializes complete refresh transactions; retest Current year/Test Connection/period switching on the replacement card before continuing the clean-room functional sequence.
...
- [ ] **Current replacement-SD position:** the clean-room run reached WU Weather commissioning and exposed a rainfall-history cache-write race. Checkpoint #58 is source/CI green; perform its focused physical retest before continuing the functional sequence.
- [ ] On the replacement spare SD, continue the final `docs/INSTALL.md` / `docs/fresh-appliance-acceptance-runbook.md` clean-room run using **`setup.sh`** through playback/EQ, AirPlay, NFC, alarm/fade and reboot after the #58 Weather retest passes.
...
5. [ ] **Run the replacement-SD release gate end-to-end.** The active run is paused at the focused checkpoint #58 WU rainfall-history retest. After that passes, continue playback/EQ, AirPlay, NFC, scheduled alarm/fade, representative reboot, all three formal verifiers, repeat `setup.sh` idempotence proof, repeat verifiers, clean `git status --porcelain`, final evidence/documentation, final full validation and explicit owner approval before PR #2 can leave Draft or merge.
