...
#### Settings numeric-control + semantic palette closure follow-up — source/CI PASS; physical re-check pending
...
- [x] Exact code/test head `8f76a8f33e770bf8dda89cd43f95cd4db2782abf` passed `pytest` run `32414179057`: compile/JavaScript/page/shell steps PASS and **1,751/1,751 unit tests PASS**.
- [ ] **Physical focused re-check:** pull this head to the working Pi, confirm the converted controls render sensibly at 1024×600, save/reload without value drift, and inspect representative non-Classic Settings pages for any fixed cyan/orange leakage. This is a focused sanity check of the new follow-up, not a reopening of the already accepted main presentation pass.
...
- **#53 — safer Settings numeric controls + semantic palette closure — SOURCE/CI PASS; PI PHYSICAL RE-CHECK PENDING.** Dashboard idle timeout, Weather auto-refresh and AirPlay pause hold now use preset selectors while preserving existing non-preset values; remaining Weather numeric fields carry explicit type/range/step constraints backed by server-side validation. A final Settings palette-closure layer moves residual non-semantic control/badge/form paint onto semantic theme variables and source regression blocks fixed-cyan reintroduction. Exact code/test head `8f76a8f33e770bf8dda89cd43f95cd4db2782abf` passed `pytest` run `32414179057` with compile/JavaScript/page/shell checks green and **1,751/1,751 unit tests PASS**. Focused 1024×600 Pi rendering/persistence/palette sanity checks remain before this checkpoint can be marked physically complete.
