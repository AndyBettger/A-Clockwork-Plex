...
### Phase 7 — full appliance installer integration — **In progress: final Weather/alarm physical pass is mostly accepted; Max gust/fade-start follow-up, Clock annunciator, final public clean-room install, verifiers and release hygiene remain**
...
#### Final Weather + alarm polish — source/CI PASS; narrow physical follow-up pending
...
- [ ] **Physical — WU-only case:** on a Pi receiving no fresh Ecowitt custom push, confirm the Clock indoor readings and Weather Indoor row disappear cleanly after the freshness window.
...
- [ ] **Physical — configurable start:** after pulling the #3669 source, set an alarm to a deliberate 10–20% Fade start volume with a 20–30 second Fade in; confirm the initial ring and Snooze re-ring both begin audibly at the configured start and ramp to the capped target.
- [ ] **Physical — WU max gust:** after pulling the #3669 source, confirm Max gust today reappears on the Clock and persists/increases as current gust observations arrive. The first updated calendar day is accepted as max-since-tracking-start; subsequent full days are complete tracked-day maxima.
