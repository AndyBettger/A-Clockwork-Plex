...
- The earlier spare/test SD developed intermittent read-only behaviour and is not valid final evidence. A replacement spare SD is now the active clean-room target; the current run has passed Plexamp/EQ commissioning, checkpoint #58 Weather rainfall-history retest, AirPlay, NFC, the real scheduled-alarm/fade/safety proof, a representative commissioned reboot and all three first-pass formal verifiers. The next gate is repeat public `setup.sh`, followed by repeat verifiers and clean-checkout proof.
...
### Phase 7 — full appliance installer integration — **In progress: all major replacement-SD functional gates plus representative reboot and first formal verifiers are physically complete; repeat public setup, repeat verifiers, clean-checkout proof and release hygiene remain**
...
- [x] **Current replacement-SD position:** the clean-room run has passed initial source/checkout identity, CamillaDSP identity, Plexamp GUI/output commissioning, Plexamp/EQ, checkpoint #58 WU rainfall-history retest, AirPlay takeover/EQ/disconnect, NFC album-tag/debounce, real scheduled-alarm fade/Snooze/re-ring/Dismiss/Music-Master-independence, the representative commissioned reboot and all three first-pass formal verifiers. The next gate is repeat public `bash setup.sh` idempotence proof.
...
5. [ ] **Finish the replacement-SD release gate.** The active run has cleared all major functional tests plus the representative reboot and first formal verifiers. Remaining physical proof is the repeat public `setup.sh` idempotence run, repeat verifiers and clean `git status --porcelain`, followed by final evidence/documentation, final full validation and explicit owner approval before PR #2 can leave Draft or merge.
