...
3. [x] **Refresh PR #2 description without changing its Draft/open state.** On 21 August the stale Stage-C/future-EQ narrative and obsolete 490-test count were replaced with the integrated release-candidate scope, current 1,762-test baseline and remaining replacement-SD/release gates. PR #2 was verified after the update as Draft, open and unmerged.
...
- **#57 — PR #2 release-candidate description refresh — PASS (metadata; Draft preserved).** On 21 August the PR body was replaced with the actual integrated appliance scope, current installer/runtime contracts, 1,762-test baseline and remaining replacement-SD/release gates. GitHub immediately confirmed `state=open`, `draft=true`, `merged=false`; no readiness or merge action was performed.
