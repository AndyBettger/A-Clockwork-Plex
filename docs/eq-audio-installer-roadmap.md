...
- The earlier spare/test SD developed intermittent read-only behaviour and is not valid final evidence. A replacement spare SD is now the active clean-room target; the current run has passed Plexamp/EQ commissioning and the checkpoint #58 Weather rainfall-history retest, with AirPlay, NFC, alarm, reboot/verifiers and repeat-install proof still to complete.
...
- [x] **Current replacement-SD position:** the clean-room run has passed initial source/checkout identity, CamillaDSP identity, Plexamp GUI/output commissioning, Plexamp/EQ functional checks and the checkpoint #58 WU rainfall-history focused retest. The next functional gate is AirPlay, followed by NFC and the real scheduled alarm/fade check.
- [ ] On the replacement spare SD, continue the final `docs/INSTALL.md` / `docs/fresh-appliance-acceptance-runbook.md` clean-room run through AirPlay, NFC, alarm/fade and representative reboot.
...
5. [ ] **Run the replacement-SD release gate end-to-end.** The active run has cleared checkpoint #58 and continues with AirPlay, NFC and the scheduled alarm/fade check, followed by representative reboot, all three formal verifiers, repeat `setup.sh` idempotence proof, repeat verifiers, clean `git status --porcelain`, final evidence/documentation, final full validation and explicit owner approval before PR #2 can leave Draft or merge.
