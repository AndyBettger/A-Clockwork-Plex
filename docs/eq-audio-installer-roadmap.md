...
- **Post-#27 documentation CI incident — FAIL at `7479d630...`, scoped repair follows.** Documentation reconciliation commit `7479d6308417561983bbde87e3a9a788686388a1` failed Tests #3413 / run `31972654807` only in two `test_fresh_appliance_acceptance_runbook.py` assertions because the test-pinned `# 14. Commission Weather Underground through Settings` heading had been expanded. Application compile, JavaScript/page wiring and shell syntax all passed, and checkpoint #27's implementation PASS is unchanged. The repair restores the exact accepted heading while retaining historical-rainfall subsections and makes Direct physical rerun logs timestamped so earlier failed-attempt evidence cannot be overwritten. Physical mutation remains gated on full CI for the repaired branch head.
...
2. Rerun the same idempotent spare-SD `--fresh-bootstrap --audio direct` apply using a new timestamped Direct evidence log. Require the already-proven package/venv/hardware/Plexamp/NFC/application stages to reconverge and the corrected protected-sudoers final verifier to let the complete Direct transaction commit with root installer exit `0`. Stop on any new unexplained host-only blocker.
