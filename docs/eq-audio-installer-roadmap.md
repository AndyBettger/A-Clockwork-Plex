...
### Phase 7 — full appliance installer integration — **In progress: functional/audio/Weather/annunciator/V3, daytime palettes, AirPlay long-form presentation and night-preview timing are physically accepted; focused Settings/Clock theme-control closure, replacement-SD clean-room install, formal verifiers and release hygiene remain**
...
- [ ] **Focused physical control-state closure:** confirm the revised non-Classic selected Alarm weekday fill, the newly unified remaining Settings pills, and removal of late legacy cyan paint from the listed Settings dropdown/button/Audio diagnostic surfaces.
...
- [x] Selected Alarm weekday presentation accepts both `.is-selected` and `aria-pressed="true"`; the final non-Classic rule now has sufficient specificity to retain the theme glow while applying solid `--accent-strong` fill and contrast text.
- [x] Shared `.settings-chip` owns compact pill geometry and final non-semantic theme paint with sufficient authority to beat late workspace CSS. This covers Weather Observation Source readiness, Alarm count and Master EQ health as well as the physically accepted Forecast/Running pills.
- [x] Non-Classic Settings dropdown triggers, ordinary action buttons/toggles and affected Audio/Alarm diagnostic surfaces now use final theme-control paint, preventing late-injected legacy cyan border/fill styles from reclaiming them. Semantic warning/error colours remain component-owned.
- [x] Main Clock colon ON colour now follows `--acp-theme-display` like the digit segments. Blink state is derived from the Clock's **rendered seconds timestamp** by observing `#clock-time` `aria-label`, eliminating the separate free-running colon timer; the same mechanism remains active on the dimmed inactive night Clock.
...
- [x] Exact source/test head `7ca1a80638ab1f69d88afe8c6e1a10c2e7a2555f` passed Tests #3891 / run `32323546470`: full workflow **PASS**, **1,729/1,729 unit tests PASS**.
- [ ] **Physical:** in a non-Classic palette such as Midnight Blue and Crimson Glow, confirm selected Alarm weekdays are now solid theme-filled **and** retain the glow; unselected days remain subdued.
- [ ] **Physical:** confirm Weather Observation Source `WU/Ecowitt ready` + `History ready`, Alarm schedule count and Audio Master EQ health now match the accepted compact Forecast/Running status-pill treatment.
- [ ] **Physical:** confirm no stale cyan paint remains on the reported Settings dropdowns/buttons/surfaces, especially Refresh forecast now; Alarm Off/Preview tone/Duplicate; Master EQ Bypass/Return to neutral; the Audio bottom status/default surface; and Advanced Audio diagnostics Test tone/Stop alarm audio.
- [ ] **Physical:** confirm main Clock colons match the digit colour and change state exactly with displayed second changes (one second on, one second off), including the dimmed inactive night Clock.
...
- [ ] After the remaining focused Settings/Clock theme-control presentation follow-up is visually accepted on usable media, run bootstrap/application/audio verifiers after a representative final reboot and commit the evidence.
...
**Phase 7 exit condition:** final V3/theme/AirPlay/Settings/Clock presentation is accepted; the replacement spare-SD appliance passes functional/formal reboot plus the final public `setup.sh` clean-room install and repeat-install; `verify-fresh-bootstrap.sh`, `verify-appliance.sh` and `scripts/audio/verify-audio.sh` are green where applicable; the repository is clean and documented; and explicit owner approval is given. PR #2 remains Draft throughout.
...
- **#49 — final theme state consistency + rendered-second Clock sync — PASS (source/CI; four focused physical groups pending).** Selected non-Classic Alarm weekdays now use a specificity-strong solid theme-accent fill plus retained glow; all non-semantic Settings status chips are normalized to the accepted compact pill contract; late runtime Settings dropdown/button/Audio diagnostic paint is forced back onto theme-control colours; and Clock colon ON colour follows the same theme display token as the digits while blink parity follows the actually rendered seconds timestamp instead of a separate timer. The first CI attempt, Tests #3889, correctly exposed two stale assertions describing the superseded chip/colon contracts; the source/runtime changes themselves were not the failure. Those assertions were updated, and exact final source/test head `7ca1a80638ab1f69d88afe8c6e1a10c2e7a2555f` passed Tests #3891 / run `32323546470` with **1,729/1,729 unit tests PASS**. Remaining bedside checks are the four groups listed in Final presentation closure above.
